...
const char *reg = "REGISTER %s SIP/2.0\n"
...
const char *invite = "INVITE sip:%s@chicago.voip.ms SIP/2.0\n"
                     "Via: SIP/2.0/UDP %s:%s;branch=%s\n"
...
const char *ack_str = "ACK %s SIP/2.0\n"
                      "Via: SIP/2.0/UDP %s:%s;branch=%s\n"
                      "From: <sip:%s@chicago1.voip.ms>;tag=%s\n"
...
const char *ok_str = "SIP/2.0 200 OK\n"
                     "Via: %s\n"
                     "From: %s\n"
                     "To: %s\n"
                     "Call-ID: %s\n"
...
int get_external_ip(char *ip_str) {
  ...
  int err = sendto(sock, &stun_request, sizeof(stun_request), 0,
  ...
  ssize_t rec_len =
      recvfrom(sock, buf, 128, 0, (struct sockaddr *)&server_addr, &socklen);
...
int send_reg(int sock, struct sockaddr_in *sip_sockaddr, char *buf, int buf_len,
             char *external_ip_str) {

  // Send the initial REGISTER request
  int send_len = snprintf(
      buf, buf_len, reg, "sip:chicago1.voip.ms", external_ip_str, "65300",
      "z9hG4bKPjeac04633294241c78541dba3d8c5461b",
      "sip:444138@chicago1.voip.ms", "263df7334c424c518c978cf068c661f5",
      "sip:444138@chicago1.voip.ms", "263df7334c424c518c978cf068c661f5",
      "sip:444138", external_ip_str, "65300");
  ...
  int err = sendto(sock, buf, send_len, 0, (struct sockaddr *)sip_sockaddr,
                   socklength);
  if (err < 0) {
  ...
  int rec_len =
      recvfrom(sock, buf, buf_len - 1, 0, (struct sockaddr *)sip_sockaddr,
  ...
  err = get_digest(digest_response, auth_ptr, username, password, digest_uri,
                   realm, nonce, method);
  if (err == 1) {
    return 1;
  }
  ...
  send_len = snprintf(
      buf, buf_len, auth_reg, "sip:chicago1.voip.ms", external_ip_str, "65300",
      "z9hG4bKPjeac04633294241c78541dba3d8c5461b",
      "sip:444138@chicago1.voip.ms", "263df7334c424c518c978cf068c661f5",
      "sip:444138@chicago1.voip.ms", "263df7334c424c518c978cf068c661f5",
      "sip:444138", external_ip_str, "65300", username, realm, nonce,
      digest_uri, digest_response, "MD5");
  ...
  err = sendto(sock, buf, send_len, 0, (struct sockaddr *)sip_sockaddr,
               socklength);
  if (err < 0) {
    ESP_LOGE(TAG, "sendto error");
    return 1;
  ...
  rec_len = recvfrom(sock, buf, buf_len - 1, 0, (struct sockaddr *)sip_sockaddr,
                     (socklen_t *)&socklength);
  ESP_LOGI(TAG, "Received %d bytes\n", rec_len);
...
int send_invite(int sock, struct sockaddr_in *sip_sockaddr, char *buf,
                int buf_len, char *external_ip_str, char *call_number) {

  int sdp_len = snprintf(buf, buf_len, sdp_str, "0", external_ip_str,
                         "my_session", external_ip_str);
  ...
  send_len = snprintf(buf, buf_len, invite, call_number, external_ip_str,
                      "65300", "z9hG4bKPjfc7170ddca184cc38e1edaac48207ce7",
                      username, "f2e78bb18e0f460e8724e9983319e1f3", call_number,
                      "6948ac89686744c8b37f0ba3ba175e1b", username,
                      external_ip_str, "65300", "3011", len_as_str);

  send_len += snprintf(buf + send_len, buf_len - send_len, sdp_str, "0",
                       external_ip_str, "my_session", external_ip_str);
  ...
  int rec_len = recvfrom(sock, buf, buf_len - 1, 0,
                         (struct sockaddr *)sip_sockaddr, &socklength);
  ESP_LOGI(TAG, "Received %d bytes\n", rec_len);
  if (rec_len > buf_len) {
    buf[buf_len - 1] = '\0';
  } else if (rec_len > 0) {
  ...
  send_len = snprintf(buf, buf_len, auth_invite, call_number, external_ip_str,
                      "65300", "z9hG4bKPjfc7170ddca184cc38e1edaac48207ce7",
                      username, "f2e78bb18e0f460e8724e9983319e1f3", call_number,
                      "6948ac89686744c8b37f0ba3ba175e1b", username,
                      external_ip_str, "65300", "3012", username, realm, nonce,
                      digest_uri, digest_response, "MD5",
                      len_as_str); // NOTE: this should be incorrect but works
                                   // anyways -- auth message gets no SDP body
...
void make_call(char *call_number) {
  ...
  int err = send_invite(sock, &sip_sockaddr, buf, BUF_LEN, external_ip_str,
                        call_number);
  if (err) {
  ...
  ssize_t rec_len = recvfrom(sock, buf, BUF_LEN - 1, 0,
                             (struct sockaddr *)&sip_sockaddr, &socklength);
  ESP_LOGI(TAG, "Received %d bytes\n", rec_len);
  if (rec_len > BUF_LEN) {
    buf[BUF_LEN - 1] = '\0';
  ...
  if (!strncmp(response_type, "180", 3)) {
    ...
    *port_end = '\0';
    ...
    rtp_init(rtp_port);
    BaseType_t tret =
        xTaskCreate(taskReceive, "TaskReceive", 4096, NULL, 1, NULL);
    if (tret != pdPASS) {
  ...
  while (1) {
    ...
    rec_len = recvfrom(sock, buf, BUF_LEN - 1, 0,
                       (struct sockaddr *)&sip_sockaddr, &socklength);
    ESP_LOGI(TAG, "Received %d bytes\n", rec_len);
    if (rec_len > BUF_LEN) {
      buf[BUF_LEN - 1] = '\0';
    ...
    if (!strncmp(buf, sip_ver, strlen(sip_ver))) {
      ...
      if (!strncmp(response_type, "200", 3)) { // OK
        ...
        send_len =
            snprintf(buf, BUF_LEN, ack_str, "sip:2245711812@208.100.60.8:5060",
                     external_ip_str, "65300",
                     "z9hG4bKPjfc7170ddca184cc38e1edaac48207ce7", username,
                     "f2e78bb18e0f460e8724e9983319e1f3", call_number, to_tag,
                     "3012", "6948ac89686744c8b37f0ba3ba175e1b");
      ...
      if (!strncmp(buf, "BYE", 3)) {
        ...
        *str_ptr = '\0';
        ...
        ESP_LOGI(TAG, "toline: %s\n", toline);
        send_len = snprintf(buf, BUF_LEN, ok_str, vialine, fromline, toline,
                            call_id, cseq);
...
void call_task(void *task_args) {
  ...
  xTaskNotify(*(TaskHandle_t *)&call_number[12], pdTRUE,
